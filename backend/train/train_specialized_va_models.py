...
specialized_va_clf_model_dir = config.specialized_va_clf_model_dir
...
specialized_va_update_sense_clf_model_dir = config.specialized_va_update_sense_clf_model_dir
...
specialized_va_root_clf_model_vectorizer = config.specialized_va_root_clf_model_vectorizer
specialized_va_root_clf_model = config.specialized_va_root_clf_model


class TrainSpecializedVaSvpModel:
    def __init__(self, selected_intent, selected_intent_svp_json_file=''):
        ...
        temp_svp_model_json_file_path = os.path.join(specialized_va_svp_model_dir_core, 'json')
    ...
    def train_svp_model(self):
        try:
            ...
            intent_path = specialized_va_svp_model_dir + '/' + intent
            if os.path.exists(intent_path):
                shutil.rmtree(intent_path)
            os.mkdir(intent_path)

            svp_model_output_path = os.path.join(specialized_va_svp_model_dir, intent)
...
class TrainSpecializedVaClassifierModel:
    ...
    def train_classifier_model(self):
        ...
        if self.selected_update_intent == 'none':
            df = pd.read_json(specialized_va_clf_model_root_intents_json_file)
            ...
            pickle.dump(count_vect, open(specialized_va_root_clf_model_vectorizer, 'wb'))

            # Save the model
            # mod_file = 'classification.model'
            pickle.dump(model, open(specialized_va_root_clf_model, 'wb'))

            print('------SVC CLASSIFICATION TRAINING COMPLETED------')
            print("Vectorizer model saved to: ", specialized_va_root_clf_model_vectorizer)
            print("Classification model saved to: ", specialized_va_root_clf_model)
            # except:
...
class TrainSpecializedVaUpdateSenseClassifierModel:
    def __init__(self):
        self.self = self
    def train_update_sense_classifier_model(self):
        # try:
        update_sense_json_file = 'update_sense.json'
        path_to_look_for = os.path.join(specialized_va_update_sense_clf_model_dir)
        if os.path.exists(path_to_look_for):
            update_sense_json_file = os.path.join(specialized_va_update_sense_clf_model_dir, 'update_sense.json') 
            update_sense_clf_model = os.path.join(specialized_va_update_sense_clf_model_dir, 'update_sense.model')
            update_sense_clf_vectorizer = os.path.join(specialized_va_update_sense_clf_model_dir, 'update_sense.pickle')
            ...
            print("Vectorizer model saved to: ", specialized_va_update_sense_clf_model_dir)
            print("Classification model saved to: ", specialized_va_update_sense_clf_model_dir)
            # except:
            #     user_message = 'Error training classification model'
            #     print(user_message)